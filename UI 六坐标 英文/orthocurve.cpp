	...
	: QMainWindow(parent)
{
	...
	setWindowTitle(tr("OrthoCurve"));
	ui.mainToolBar->setStyleSheet("background-color: rgba(255, 255, 255, 255);");
	ui.mainToolBar->setIconSize(QSize(40, 40));

	// set mainmenu icon text size
	for (int i = 0; i < ui.mainToolBar->actions().size(); ++i) {
		ui.mainToolBar->actions().at(i)->setFont(QFont("Microsoft YaHei", 10));
	}
	...
	ShowMessage("Pls import STL model file");
	LoadSetting();
	m_editCurve = NULL;
	m_editMark  = NULL;
	...
	projectCurveProgressDialog->setLabelText("Loading model...");
...
void OrthoCurve::Init()
{
	...
	prevButton = createToolButton(tr("previous model"), QIcon(prevName), SLOT(OnPreview()));
	nextButton = createToolButton(tr("next model"), QIcon(nextName), SLOT(OnNext()));

	// craete view side buttons
	viewSideButtons.resize(6);
	viewSideButtons[0] = createToolButton(tr("up view"),	QIcon(appPath	+ "/Resources/up.png"),			SLOT(OnUp()));
	viewSideButtons[1] = createToolButton(tr("down view"),	QIcon(appPath	+ "/Resources/down.png"),		SLOT(OnDown()));
	viewSideButtons[2] = createToolButton(tr("left view"),	QIcon(appPath	+ "/Resources/left.png"),		SLOT(OnLeft()));
	viewSideButtons[3] = createToolButton(tr("right view"), QIcon(appPath	+ "/Resources/right.png"),		SLOT(OnRight()));
	viewSideButtons[4] = createToolButton(tr("front view"), QIcon(appPath	+ "/Resources/front.png"),		SLOT(OnFront()));
	viewSideButtons[5] = createToolButton(tr("back view"),	QIcon(appPath	+ "/Resources/back.png"),		SLOT(OnBack()));
...
void OrthoCurve::OnImportSTLs()
{
	...
	if (m_models.Count() > 0)
	{
		...
		QMessageBox box;
		box.setText(tr("Successfully Import"));
		box.exec();

		QString message;
		message = QString("Successfully import ") + QString::number(m_models.Count()) + QString(" models");
		ShowMessage(message);
		m_currentID = -1;
...
void OrthoCurve::OnSaveCurve()
{
	...
	if (num == 0) {
		QMessageBox box;
		box.setText(tr("There is no model"));
		box.exec();
		return;
	}
	...
	if (m_savePath.mid(m_savePath.lastIndexOf('.')) == QString(".txt")) {
		file_path = QFileDialog::getSaveFileName(this, "Pls select path to save the the cut curve...", m_savePath, tr("text(*.txt);;points(*.pts)"));
	}
	else {
		file_path = QFileDialog::getSaveFileName(this, "Pls select path to save the the cut curve...", m_savePath, tr("points(*.pts);;text(*.txt)"));
	}
...
void OrthoCurve::OnPreview()
{
	...
	QString message;
	message = QString("Total of ") + QString::number(m_models.Count()) + QString(" models，the current one is the ") + QString::number(m_currentID + 1) + QString(" model.");
	if (m_currentID == 0)
		message += QString("the next model is to click the next one button.");
	else if (m_currentID + 1 >= m_models.Count())
		message += QString("the last model is to click the last one button.");
	else
		message += QString("click next or last one button to selcect related model.");

	if (m_isManual) {
		message += QString("The mode is manually selected.【Left click control point and drag】Change control point position 【Right click or click delete last point button】Select and delete control point");
	}
...
void OrthoCurve::OnMarkIcon()
{
	...
	message = QString("【double left click】on model to pick laser mark position,【double left click】on related laser mark position to delete it.");
	ShowMessage(message);
}
