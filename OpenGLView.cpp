...
void OpenGLView::drawForeground(QPainter *pPainter, const QRectF &rc)
{
	...
	if (m_scene && m_scene->m_customObjects.size() >= 1) {
		...
		QString model_name(tr(m_scene->m_customObjects[0]->GetName()));
		QStringList model_names = model_name.split('_');
		QString model_name_print = model_name;
		if (model_names.size() >= 3) {
			model_name_print = model_names.at(0) + model_names.at(1) + QString("%1").arg(model_names.at(2), 2, QLatin1Char('0'));
		}
		text += model_name_print;
